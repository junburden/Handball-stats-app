...
function getTeamsForGame(game_id) {
  $.get('/teams.json', {game_id: game_id})
  .done(function(data) {
...
function shooterDiv(value, name, number) {
  return $("<div class='col-sm-6'><label data-value='" + value + "' for='shot_shooter_id_" + value + "'><input type='radio' value='" + value + "' name='shot[shooter_id]' id='shot_shooter_id_" + value + "'> " + number + " " + name + "</label></div>")
}

function getPlayersForShooters(team_id) {
  $.get('/players.json', {team_id: team_id})
  .done(function(data) {
